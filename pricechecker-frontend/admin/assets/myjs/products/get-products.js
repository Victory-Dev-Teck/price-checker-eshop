...
async function getWalmartProducts(maxNumberOfProducts, currentPage){
    let resultHtml = '';
    let response = {};
    if(isMockData){
    ...
    if(respLen){
            ...
            for(i = 0; i < respLen; i++){
                ...
                resultHtml +=  '<tr>\n' +
                    ...
                    '                                            <a data-toggle="tooltip" title="Add to our system" class="pd-setting-ed"><i class="fa fa-plus" aria-hidden="true" onclick="registerProduct(\'' + title + '\',\'' + description + '\',\'' + product_url + '\',\'' + image_url + '\',\'' + price + '\',\'' + category + '\',\'' + eshop +'\')" ' +
...
async function getAllProducts(){
    ...
    var walmartProducts = await getWalmartProducts(numOfShowProducts, currentPageOfWalmart)
...
async function getPreviewPageEbay(){
    if(currentPageOfEbay > 0){
        ...
        var element = document.getElementById('product-table-Ebay');
...
async function getNextPageWalmart( ){
    ...
    var resultHtml =  await getWalmartProducts(numOfShowProducts, currentPageOfWalmart);
    $("#product-table-amazon").html(resultHtml);
}
async function getPreviewPageWalmart(){
    if(currentPageOfWalmart > 0){
        currentPageOfEbay --;
        var resultHtml =  await getWalmartProducts(numOfShowProducts, currentPageOfWalmart);
        var element = document.getElementById('product-table-Ebay');
